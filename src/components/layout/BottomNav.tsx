  ...
  return (
    <nav className="fixed bottom-0 w-full max-w-md md:hidden bg-white border-t border-slate-200 px-8 py-4 flex justify-between items-center z-20 pb-10 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
      <button 
        ...
        className={`flex flex-col items-center gap-1 ${pathname === '/home' || pathname === '/' ? 'text-red-600' : 'text-slate-300'}`}
      >
        <Home size={22} />
        <span className="text-[9px] font-black uppercase">Home</span>
      </button>

      <button onClick={() => router.push('/aisles')} className={`flex flex-col items-center gap-1 ${pathname.startsWith('/aisles') ? 'text-red-600' : 'text-slate-300'}`}>
        <Grid size={22} />
      ...
      >
        ...
        <span className="text-[10px] font-black leading-none">R{total.toFixed(0)}</span>
        {count > 0 && (
      ...
      <button onClick={() => router.push('/sales')} className={`flex flex-col items-center gap-1 ${pathname.startsWith('/sales') ? 'text-red-600' : 'text-slate-300'}`}>
        <Tag size={22} />
